...
void	image_to_window(t_main *cs)
{
	...
	if (cs->ppos.hand == 3)
		mlx_put_image_to_window(cs->data.mlx, cs->data.mlx_win, cs->ass.m9.img, WIND_W * 0.01, (WIND_H/2) * 1.1);
	else if (cs->ppos.hand == 2)
		mlx_put_image_to_window(cs->data.mlx, cs->data.mlx_win, cs->ass.deagle.img, WIND_W/2, (WIND_H/2) * 1.19);
	else if (cs->ppos.hand == 1)
		mlx_put_image_to_window(cs->data.mlx, cs->data.mlx_win, cs->ass.ak.img, WIND_W/2, (WIND_H/2) * 1.08);
}
