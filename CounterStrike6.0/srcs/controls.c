...
int	mouse_mov(int x, int y, t_main *cs)
{
	...
	{
		if (x > WIND_W/2)
			look_right(cs, (x - WIND_W/2) * X_SENSE);
		else if (x < WIND_W/2)
			look_left(cs, (WIND_W/2 - x) * X_SENSE);
		if (y < WIND_H / 2)
			look_up(cs, ((WIND_H/2) - y) * Y_SENSE);
		else if (y > WIND_H / 2)
			look_down(cs, (y - (WIND_H / 2)) * Y_SENSE);
	}
...
int	press_binds(int key, t_main *cs)
{
	if (key == 53)
		...
		cs->ppos.j.j_time = current_timestamp();
	return (0);
}
