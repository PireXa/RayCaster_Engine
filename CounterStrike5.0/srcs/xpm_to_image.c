...
t_img	converter(t_main *cs, char *img)
{
	...
	if (!ft_strcmp(img, "ak"))
	{
		xpm.img = mlx_xpm_file_to_image(cs->data.mlx, "./assets/skins/ak.xpm", &cs->ass.ak.x, &cs->ass.ak.y);
		
		return(xpm);
	}
	else if (!ft_strcmp(img, "m9"))
	{
		xpm.img = mlx_xpm_file_to_image(cs->data.mlx, "./assets/skins/m9.xpm", &cs->ass.m9.x, &cs->ass.m9.y);
		return (xpm);
	}
